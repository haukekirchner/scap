...
\begin{block}{Hints}
    \begin{itemize}
        \item Set number of epochs to one
        \begin{itemize}
            \item in \texttt{/code/train.py} $\rightarrow$ \texttt{epoch = 1}
        \end{itemize}
            \item decrease time requested in SLURM
        \begin{itemize}
...
\begin{block}{Backup}
\vspace{-0.5em}
\begin{lstlisting}[breaklines=true,numbers=none,frame=none,basicstyle=\scriptsize]
# download example profiling output 
rsync -avvH <user>@glogin9.hlrn.de:/scratch/projects/\
workshops/gpu-workshop/profiling_example ~/Downloads
